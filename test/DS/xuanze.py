...
def save_to_json(file_path, data):
    ...
    try:
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            json.dump(data, f, ensure_ascii=False, indent=4)  # 直接覆盖写入
...
def save_to_excel(file_path, data):
    ...
    try:
        ...
        df.to_excel(file_path, index=False, engine='openpyxl')  # 直接覆盖写入
...
def loadData(json_file_path, output_excel_path, output_json_path, batch_size, api_keys):
    try:
        ...
        for i in range(0, len(data), batch_size):
            ...
            queries = [item.get("query", "") for item in batch]  # 默认空字符串
            answers = [item.get("answer", "") for item in batch]  # 默认空字符串
...
if __name__ == '__main__':
    # 设置多个 API 密钥
    api_keys = [
        "sk-279e9f7b46e948afb458f47cd0652eb1"
    ]

    # 分类任务
    # test_json_path = "/root/autodl-tmp/PIXIU/data/选择任务/概念单选/test.json"
    # out_json_path = "/root/autodl-tmp/ds-r1测评/data/result/json/分类任务/Audit_single_choice.json"
    # out_excel_path = "/root/autodl-tmp/ds-r1测评/data/result/excel/分类任务/Audit_single_choice.xlsx" 欧克 晚

    # test_json_path = "/root/autodl-tmp/PIXIU/data/选择任务/概念判断/test.json"
    # out_json_path = "/root/autodl-tmp/ds-r1测评/data/result/json/选择任务/Audit_concept_judge.json"
    # out_excel_path = "/root/autodl-tmp/ds-r1测评/data/result/excel/选择任务/Audit_concept_judge.xlsx"  ok 晚

    test_json_path = "/root/autodl-tmp/PIXIU/data/选择任务/计算选择题/test.json"
    out_json_path = "/root/autodl-tmp/ds-r1测评/data/result/json/选择任务/Audit_calc_choice.json"
    out_excel_path = "/root/autodl-tmp/ds-r1测评/data/result/excel/选择任务/Audit_calc_choice.xlsx" ok 晚
