...
def clean_response(response, query):
    ...
    # 移除特定的前缀部分
    prefix = "system You are Qwen, created by Alibaba Cloud. You are a helpful assistant. user assistant "
    ...
    # 再检查是否包含“答案：”或“答：”并处理
    answer_prefixes = ["system You are Qwen, created by Alibaba Cloud. You are a helpful assistant. user assistant ", 
                       "答案：", "答：", "审计案例：", "题目："]  # 定义可能的前缀列表
...
class Audit_Standard(QwenTask):
    def __init__(self, model_name, batch_size):
        ...
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/excel/自动问答/Audit_Standard.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/json/自动问答/Audit_Standard.json"
...
class Audit_Target(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        self.task_name = "Audit_Target"
        self.json_file_path = "/root/autodl-tmp/test_models/new_data/自动问答/审计目标问答/audit_target.json"
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_7B_Instruct/excel/自动问答/audit_target.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_7B_Instruct/json/自动问答/audit_target.json"

    def run(self):
        self.generate_responses(self.json_file_path, self.output_excel_path, self.output_json_path)
        self.evaluate(self.output_json_path, self.task_name)

class Audit_law(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        self.task_name = "Audit_law"
        self.json_file_path = "/root/autodl-tmp/test_models/new_data/自动问答/审计法规问答/test.json"
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/excel/自动问答/Audit_law.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/json/自动问答/Audit_law.json"

    def run(self):
        self.generate_responses(self.json_file_path, self.output_excel_path, self.output_json_path)
        self.evaluate(self.output_json_path, self.task_name)

class Audit_conception(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        self.task_name = "Audit_conception"
        self.json_file_path = "/root/autodl-tmp/test_models/new_data/自动问答/审计概念问答/QA_concept1.json"
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/excel/自动问答/Audit_conception1.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/json/自动问答/Audit_conception1.json"

    def run(self):
        self.generate_responses(self.json_file_path, self.output_excel_path, self.output_json_path)
        self.evaluate(self.output_json_path, self.task_name)

class Audit_content(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        self.task_name = "Audit_content"
        self.json_file_path = "/root/autodl-tmp/test_models/new_data/自动问答/审计内容问答/audit_content.json"
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/excel/自动问答/Audit_content.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/json/自动问答/Audit_content.json"
...
class Audit_problem_summary(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        ...
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_7B_Instruct/excel/文本生成/Audit_problem_summary.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_7B_Instruct/json/文本生成/Audit_problem_summary.json"

    def run(self):
        self.generate_responses(self.json_file_path, self.output_excel_path, self.output_json_path)
        self.evaluate(self.output_json_path, self.task_name)

class Audit_material_analyse(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        ...
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/excel/文本生成/Audit_material_analyse.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/json/文本生成/Audit_material_analyse.json"

    def run(self):
        self.generate_responses(self.json_file_path, self.output_excel_path, self.output_json_path)
        self.evaluate(self.output_json_path, self.task_name)

class Audit_case_generation(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        self.task_name = "Audit_case_generation"
        self.json_file_path = "/root/autodl-tmp/test_models/new_data/文本生成/审计案例撰写/test.json"
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/excel/文本生成/Audit_case_generation.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/json/文本生成/Audit_case_generation.json"

    def run(self):
        self.generate_responses(self.json_file_path, self.output_excel_path, self.output_json_path)
        self.evaluate(self.output_json_path, self.task_name)

class Audit_matters(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        self.task_name = "Audit_matters"
        self.json_file_path = "/root/autodl-tmp/test_models/new_data/问题定性/审计事项/test.json"
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/excel/问题定性/Audit_matters.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/json/问题定性/Audit_matters.json"

    def run(self):
        self.generate_responses(self.json_file_path, self.output_excel_path, self.output_json_path)
        self.evaluate(self.output_json_path, self.task_name)

class Audit_manifestations(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        ...
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/excel/问题定性/Audit_manifestations.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/json/问题定性/Audit_manifestations.json"

    def run(self):
        self.generate_responses(self.json_file_path, self.output_excel_path, self.output_json_path)
        self.evaluate(self.output_json_path, self.task_name)

class simple_Qualitative_basis(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        ...
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/excel/问题定性/simple_Qualitative_basis.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/json/问题定性/simple_Qualitative_basis.json"

    def run(self):
        self.generate_responses(self.json_file_path, self.output_excel_path, self.output_json_path)
        self.evaluate(self.output_json_path, self.task_name)

class Qualitative_basis(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        ...
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/excel/问题定性/Qualitative_basis.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/json/问题定性/Qualitative_basis.json"

    def run(self):
        self.generate_responses(self.json_file_path, self.output_excel_path, self.output_json_path)
        self.evaluate(self.output_json_path, self.task_name)

class simple_penalty_basis(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        ...
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/excel/问题定性/simple_penalty_basis.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5_14B_Instruct/json/问题定性/simple_penalty_basis.json"

    def run(self):
        self.generate_responses(self.json_file_path, self.output_excel_path, self.output_json_path)
        self.evaluate(self.output_json_path, self.task_name)

class penalty_basis(QwenTask):
    def __init__(self, model_name, batch_size):
        super().__init__(model_name, batch_size)
        self.task_name = "penalty_basis"
        self.json_file_path = "/root/autodl-tmp/test_models/new_data/问题定性/问题处罚依据/penalty_test.json"
        self.output_excel_path = "/root/autodl-tmp/test_models/result/Qwen2.5-14B-Instruct/excel/问题定性/penalty_basis.xlsx"
        self.output_json_path = "/root/autodl-tmp/test_models/result/Qwen2.5-14B-Instruct/json/问题定性/penalty_basis.json"
...
# 主函数
def main():
    model_name = "/root/autodl-tmp/PIXIU/models/Qwen/Qwen2.5-7B-Instruct"  # 模型名称
    # model_name = "/root/autodl-tmp/PIXIU/models/Qwen/Qwen2.5-14B-Instruct"  # 模型名称
    batch_size = 32 # 调整批量大小以减少 CUDA 内存占用

    # 创建并运行具体任务
    audit_task = Audit_material_analyse(model_name, batch_size)
    audit_task.run()
